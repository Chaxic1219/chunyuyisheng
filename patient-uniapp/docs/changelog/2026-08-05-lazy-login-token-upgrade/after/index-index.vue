...
import { resolveHomeQuickIcon, resolveIconSrc, safeLocalImageSrc } from "../../utils/mediaSrc";
...
/** 访客态安全兜底：feed 为空时提供默认空结构，避免模板遍历 undefined 报错 */
const safeFeed = computed<any>(() => feed.value || {
  plan: null,
  alert: null,
  softNotice: null,
  pendingRecord: null,
...
const emptyStateIconSrc = computed(() => resolveIconSrc("asset-records", "#176B52"));
...
async function loadHome(force = false) {
  // 懒登录：首页加载不强制登录。未登录 → 静默展示访客态；
  // 点击「咨询/档案/健康计划」等功能时由 ensureLogin 触发绑定。
  if (!getMpToken()) {
    homeStore.reset();
    homeStore.error = "";
    return;
...
<template>
  <view class="page ambient-bg" :class="{ elder: store.elderMode }">
    ...
    <scroll-view v-else scroll-y class="home-scroll">
      <view
      ...
      >
        <view class="greeting">
          ...
          <view class="greeting__bell pressable" aria-role="button" @click="onNotify">
            <AppIcon name="clock" :size="24" color="#176B52" />
          </view>
        </view>
        ...
        <!-- 图二：高优异常 -->
        <view v-if="homeMode === 'abnormal' && safeFeed.alert" class="alert-card">
          <view class="alert-card__label-row">
            <AppIcon name="help" :size="19" color="#FFFFFF" />
            <text class="alert-card__label">{{ safeFeed.alert.label }}</text>
          </view>
          <text class="alert-card__title">{{ safeFeed.alert.title }}</text>
          <text class="alert-card__desc">{{ safeFeed.alert.desc }}</text>
          <view class="alert-card__actions">
            <text
              class="alert-card__btn alert-card__btn--primary pressable"
              aria-role="button"
              @click="go(safeFeed.alert.primaryUrl, safeFeed.alert.primaryUrl.includes('/consult/'))"
            >{{ safeFeed.alert.primaryText }}</text>
            <text
              class="alert-card__btn alert-card__btn--ghost pressable"
              aria-role="button"
              @click="go(safeFeed.alert.secondaryUrl)"
            >{{ safeFeed.alert.secondaryText }}</text>
          </view>
        </view>

        <!-- 图一：轻提醒 -->
        <view
          v-else-if="homeMode === 'task' && safeFeed.softNotice"
          class="soft-notice pressable"
          aria-role="button"
          @click="safeFeed.softNotice.actionUrl && go(safeFeed.softNotice.actionUrl)"
        >
          <AppIcon name="help" :size="19" color="#9A6418" />
          <text class="soft-notice__text">{{ safeFeed.softNotice.text }}</text>
        </view>

        <!-- 图一 / 图二：深绿计划主卡 -->
        <view v-if="safeFeed.plan" class="plan-hero">
          <view class="plan-hero__top">
            <text class="plan-hero__kicker">
              {{ homeMode === "abnormal" ? safeFeed.plan.completionText : `健康计划 · ${safeFeed.plan.completionText}` }}
            </text>
            <view class="plan-hero__tag">
              <AppIcon name="user" :size="14" color="#D8F0E4" />
              <text class="plan-hero__tag-text">{{ safeFeed.plan.modeTag || "自主管理" }}</text>
            </view>
          </view>
          <text class="plan-hero__title">{{ safeFeed.plan.title }}</text>
          <view class="plan-hero__task">
            <view class="plan-hero__task-icon">
              <AppIcon name="heart" :size="22" color="#176B52" />
            </view>
            <view class="plan-hero__task-copy">
              <text class="plan-hero__task-title">{{ safeFeed.plan.nextTask.title }}</text>
              <text class="plan-hero__task-desc">{{ safeFeed.plan.nextTask.desc }}</text>
            </view>
            <text class="plan-hero__task-btn pressable" aria-role="button" @click="onNextTask">
              {{ safeFeed.plan.nextTask.actionText }}
            </text>
          </view>
          <view class="plan-hero__foot">
            <view class="plan-hero__progress">
              <text class="plan-hero__progress-label">{{ safeFeed.plan.progressLabel || `本周完成率 ${safeFeed.plan.completionPercent}%` }}</text>
            ...
            <text
              ...
              aria-role="button"
              @click="go(safeFeed.plan.actionUrl)"
            >{{ safeFeed.plan.actionText }}</text>
          </view>
        </view>
        ...
        <view v-else-if="homeMode === 'empty'" class="home-empty home-001">
          <view class="home-empty__icon">
            <image class="home-empty__icon-img" :src="emptyStateIconSrc" mode="aspectFit" />
          </view>
        ...
        <!-- 图一：待确认档案 -->
        <view v-if="homeMode === 'task' && safeFeed.pendingRecord" class="home-block">
          <AppSectionHeader title="健康档案 · 1 份待确认" action="进入健康档案" @action="go(safeFeed.pendingRecord.actionUrl)" />
          <view class="pending-record pressable" aria-role="button" @click="go(safeFeed.pendingRecord.actionUrl)">
            <view class="pending-record__icon"><AppIcon name="file" :size="24" color="#B7791F" /></view>
            <view class="pending-record__copy">
              <text class="pending-record__title">{{ safeFeed.pendingRecord.title }}</text>
              <text class="pending-record__desc">{{ safeFeed.pendingRecord.desc }}</text>
            </view>
            <text class="pending-record__btn">{{ safeFeed.pendingRecord.actionText || "继续确认" }}</text>
          </view>
        </view>

        <!-- 快捷操作 / 你还可以：task + empty；异常态不占主视线，仍保留底部入口可按需加 -->
        <view v-if="homeMode !== 'abnormal'" class="home-block">
          <AppSectionHeader :title="safeFeed.quickActionsTitle || (homeMode === 'empty' ? '你还可以' : '快捷操作')" />
          <view class="quick-grid">
            <AppActionTile
              v-for="action in safeFeed.quickActions"
              :key="action.key"
              :icon="resolveHomeQuickIcon(action.key, action.icon)"
              :title="action.label"
              compact
        ...
        <!-- 进行中服务 / 医生管家服务 -->
        <view v-if="safeFeed.serviceProgress && homeMode !== 'empty'" class="home-block">
          <AppSectionHeader
            :title="safeFeed.serviceSectionTitle || '正在进行的服务'"
            :action="safeFeed.serviceSectionAction || '查看全部'"
            @action="go(safeFeed.serviceProgress.actionUrl)"
          />
          <view class="service-card pressable" aria-role="button" @click="go(safeFeed.serviceProgress.actionUrl)">
            <view class="service-card__avatar">{{ safeFeed.serviceProgress.providerShortName }}</view>
            <view class="service-card__copy">
              <text class="service-card__title">{{ safeFeed.serviceProgress.title }}</text>
              <text class="service-card__desc">{{ safeFeed.serviceProgress.desc }}</text>
              <text class="service-card__meta">{{ safeFeed.serviceProgress.meta }}</text>
            </view>
            <view v-if="safeFeed.serviceProgress.unreadCount" class="service-card__badge">
              {{ safeFeed.serviceProgress.unreadCount > 99 ? "99+" : safeFeed.serviceProgress.unreadCount }}
            </view>
          </view>
        </view>

        <!-- 异常说明条 -->
        <view v-if="homeMode === 'abnormal' && safeFeed.notice" class="home-notice">
          <AppIcon name="help" :size="19" color="#9A6418" />
          <text class="home-notice__text">{{ safeFeed.notice }}</text>
        </view>

        <!-- 推荐：异常态不展示 -->
        <view v-if="homeMode !== 'abnormal' && safeFeed.recommendations?.length" class="recommend-section">
          <AppSectionHeader
            :title="safeFeed.recommendationsTitle || (homeMode === 'empty' ? '常用健康服务' : '根据当前计划推荐')"
            action="全部服务"
            @action="go('/pages/services/index')"
          />
          <view class="recommend-row">
            <view
              v-for="item in safeFeed.recommendations"
              :key="item.key"
...
.elder .home-empty__icon {
  width: 64px;
  height: 64px;
}
.elder .home-empty__icon-img {
  width: 52px;
  height: 52px;
}
.home-empty__title,
