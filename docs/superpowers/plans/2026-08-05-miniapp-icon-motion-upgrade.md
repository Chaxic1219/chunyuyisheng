...
**Architecture:** 使用集中式图标注册表把页面语义映射到本地 PNG，并由 `AppIcon` 统一解析资源和动效类型；`AppButton`、`AppIconButton`、列表、卡片和页头统一处理按下、加载、成功、失败及低动效状态。页面只传递动作语义和业务状态，旧图标名称通过临时别名兼容，全部迁移后再删除。
...
- 设计依据：`docs/superpowers/specs/2026-08-05-miniapp-icon-motion-upgrade-design.md`
- 实施目录：`patient-uniapp`
...
为 `ICON_NAMES` 中每个名称导出 `96×96` 透明 PNG 到 `src/static/icons/v2/`；严格按 `TONE_FILES` 输出对应的 `-inverse`、`-muted` 和 `-danger` 文件。所有母版使用 24×24 画布和 1.8px 圆角描边，不加入底色、阴影或渐变。
