...
const { resolvePosterAsset, resolvePosterAssetByCode, resolveOutboundImageAsset } = media;
...
function prepareDelivery(doctorId, reply, patientName, options){
  ...
  const feedVideos = ((reply && reply.responses) || [])
    .filter((item)=> item && item.type === "feed_video" && item.feedVideo)
    .map((item)=> item.feedVideo);
  // 图片素材（素材库 image 类型）：触发后以图片消息发送。
  const images = ((reply && reply.responses) || [])
    .filter((item)=> item && item.type === "image" && (item.url || item.imageUrl))
    .map((item)=> ({ url: String(item.imageUrl || item.url || "").trim() }));
...
/** 入群欢迎正文：仅取 join 编排里的 text 资产，与素材库「入群欢迎」一致。
 *  mp / link 由 weappCodes、linkCards 单独投递；勿用 prepareDelivery.replyText（模板未就绪时会拼入【医患联络表】等 mp 标题行）。 */
function joinWelcomeTextFromResponses(responses, patientName){
  const textResponses = (Array.isArray(responses) ? responses : []).filter(r => r && r.type === "text");
  return responsesToQiweText(
    { responses: textResponses },
    patientName,
    { omitPatientName: true, omitMiniPrograms: true, omitLinkCards: true, omitQr: true }
  );
}
...
async function deliverReplyToQiwe(input){
  ...
  textRichDelayMs = await delayTextBeforeRich(sentParts, !!plan.weappReady || (plan.linkCards || []).length > 0 || (plan.feedVideos || []).length > 0 || !!poster);
  for(const template of (plan.feedVideos || [])){
    const r = await qiwe.sendFeedVideo(toId, template, cfg);
    sentParts.push({ type:"feed_video", feedId:template.feedId, channelName:template.channelName });
    results.push({ type:"feed_video", result:r });
  }
  // 图片素材发送（素材库 image 类型）：sendImage 支持 URL 上传兜底；单张失败不中断后续。
  for(const img of (plan.images || [])){
    try{
      const asset = resolveOutboundImageAsset(img.url);
      if(!asset) throw new Error("图片文件不存在");
      const r = await qiwe.sendImage(toId, asset, cfg);
      sentParts.push({ type:"image", url:img.url });
      results.push({ type:"image", result:r });
    }catch(e){
      console.error("[qiwe] sendImage 失败，跳过该图片素材：", img.url, "-", e.message);
      sentParts.push({ type:"image_error", url:img.url, error:e.message });
      results.push({ type:"image_error", error:e.message });
    }
  }
...
async function deliverOutbox(row, cfg){
  ...
  // 欢迎语：payload.qiwe.weappCodes 入队时已由 join/prepareDelivery 或 legacy welcomeWeappPayload 写好。
  // 出站编排可含非 979/808 编号，此处按 payload 原样投递（仅去空+去重+联络表别名），勿再硬过滤。
  ...
  for(const img of images){
    try{
      const asset = resolveOutboundImageAsset(img && img.url);
      if(!asset) throw new Error("图片文件不存在");
      const r = await qiwe.sendImage(toId, asset, cfg);
      externalMsgId = pickExternalMsgId(r) || externalMsgId;
      sentParts.push({ type:"image", url:img.url });
    }catch(e){
      linkErrors.push({ type:"image_error", url:String(img && img.url || ""), error:e.message });
    }
  }
