...
async function extractIntake(text, ctx, attachments){
  ...
  const cfg = modelConfig({ multimodal:hasImages });
  if(!cfg || process.env.TRIAGE_AI_DISABLED === "1") return null;
  ...
  const body = {
    model:cfg.model,
    messages:[ { role:"system", content:system }, { role:"user", content:userContentForModel(text, attachments) } ],
  ...
  try{
    ...
    clearTimeout(timer);
    if(!res.ok) throw new Error(cfg.errorPrefix+" intake HTTP "+res.status);
...
async function recheckReplyLLM(replyText){
  ...
  if(!cfg || process.env.TRIAGE_AI_DISABLED === "1") return true;   // 无可用模型 → fail-closed 判降级（低危生成本就需 key，理论不到此）
  const controller = new AbortController();
  const timer = setTimeout(()=>controller.abort(), +(process.env.TRIAGE_AI_TIMEOUT_MS || 8000));
  const system = [
  ...
  const body = {
    model:cfg.model,
    messages:[ { role:"system", content:system }, { role:"user", content:String(replyText || "").slice(0, 400) } ],
    thinking:{type:"disabled"},
    temperature:0,
    top_p:0.9,
    stream:false
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) return true;   // HTTP 错 → fail-closed 降级
    const data = await res.json();
    const ans = String((((data.choices||[])[0]||{}).message||{}).content || "").trim();
...
async function generateLowRiskReply({ text, ctx, patientProfileBlock, codeMenu, evidence, history, channel }){
  const cfg = modelConfig({});
  if(!cfg || process.env.TRIAGE_AI_DISABLED === "1") return { ok:false, reason:"model_unavailable" };
  const controller = new AbortController();
  const timer = setTimeout(()=>controller.abort(), +(process.env.TRIAGE_AI_TIMEOUT_MS || 8000));
  const d = (ctx && ctx.doctor) || {};
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) return { ok:false, reason:"model_http_" + res.status };
    const data = await res.json();
    const raw = String((((data.choices||[])[0]||{}).message||{}).content || "");
...
function modelConfig(opts){
  opts = opts || {};
  const prefer = String(process.env.TRIAGE_PROVIDER || "").trim().toLowerCase();
  ...
  function mimoCfg(){
    ...
    const model = opts.multimodal
      ? (process.env.MIMO_MULTIMODAL_MODEL || DEFAULT_MIMO_MULTIMODAL_MODEL)
  ...
  // TRIAGE_PROVIDER 可强制指定；默认优先 DeepSeek（MiMo 现网密钥失效时仍可出针对性回复）
  if(prefer === "deepseek") return deepseekCfg() || mimoCfg();
  if(prefer === "mimo") return mimoCfg() || deepseekCfg();
  return deepseekCfg() || mimoCfg();
}
...
async function generateAssistantReviewDraft(input){
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) return { ok:false, reason:"model_http_" + res.status, contextScope };
    const data = await res.json();
    ...
    clearTimeout(timer);
    return { ok:false, reason:"model_error", contextScope };
  }
}
...
function retrieveKnowledgeLocal(ctx, text, topK){
  ...
  const scored = items.map(k=>{
    ...
    return { ...k, raw:hit, score: hit * (KB_LAYER_WEIGHT[k.layer] || 1) * kbSourceWeight(k.source) };
  }).filter(x=>x.raw > 0).sort((a,b)=>b.score - a.score).slice(0, topK || 3);
...
async function callModel(text, ctx, scan, opts){
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) throw new Error(cfg.errorPrefix+" HTTP "+res.status);
    const data = await res.json();
...
async function assessRiskLLM(text, ctx){
  ...
  if(!t) return null;
  const cfg = modelConfig({});   // 决策4：text-only（图片风险由 withAttachmentRisk 升 medium 地板兜底，不在此判多模态）
  if(!cfg || process.env.TRIAGE_AI_DISABLED === "1") return null;   // 退化：无 key / 显式关 → null（全链路 floor-only）
  const controller = new AbortController();
  const timer = setTimeout(()=>controller.abort(), +(process.env.TRIAGE_AI_TIMEOUT_MS || 8000));
  const system = [
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) throw new Error(cfg.errorPrefix+" risk HTTP "+res.status);
    const data = await res.json();
    const raw = String((((data.choices||[])[0]||{}).message||{}).content || "");
    let obj = null;
    try{ obj = parseJsonObject(raw); }catch(e){ return null; }
...
async function classifyIntent(doctorId, text){
  ...
  const cfg = modelConfig({});
  if(!cfg || process.env.TRIAGE_AI_DISABLED === "1") return blank; // 无可用模型→回落分诊
  const controller = new AbortController();
  const timer = setTimeout(()=>controller.abort(), +(process.env.TRIAGE_AI_TIMEOUT_MS || 8000));
  const menu = [
  ...
  try{
    ...
    const raw = String((((data.choices||[])[0]||{}).message||{}).content || "");
    let obj = null;
    try{ obj = parseJsonObject(raw); }catch(e){ return blank; }
...
async function assessLevelLLM(text, ctx){
  ...
  const timer = setTimeout(()=>controller.abort(), +(process.env.TRIAGE_AI_TIMEOUT_MS || 8000));
  const system = [
    "你是医患群「消息分档」助手。只判断这条患者消息的风险档位信号，绝不输出诊断、用药、检查解读、患者话术或任何医学建议。",
    "判级要点：high=急症/红旗；medium=需关注症状或咨询；low=常见轻症/科普。拿不准时偏保守判 medium 且 needsDoctor=false。",
    "只输出一个 JSON 对象，不要任何解释或代码块。键固定为：riskLevel、needsDoctor、reason。",
    "riskLevel：low | medium | high。needsDoctor：boolean（medium 时 true→需医生，false→需医助）。reason：一句话判级依据，≤80字。",
    layeredSafetyPromptBlock(ctx && ctx.doctor && ctx.doctor.id, ctx && ctx.doctor),
  ...
  try{
    const res = await fetch(cfg.url, { method:"POST", headers:cfg.headers, signal:controller.signal, body:JSON.stringify(body) });
    clearTimeout(timer);
    if(!res.ok) throw new Error(cfg.errorPrefix+" level HTTP "+res.status);
    const data = await res.json();
    const raw = String((((data.choices||[])[0]||{}).message||{}).content || "");
    let obj = null;
    try{ obj = parseJsonObject(raw); }catch(e){ return null; }
    return coerceLevelAssessment(obj);
  }catch(e){
    clearTimeout(timer);
    return null;
  }
...
async function resolveMessageLevelAsync(text, doctorId, ctx){
  ...
  if(aiLevelClassifierEnabled()){
    const raw = await module.exports.assessLevelLLM(text, ctx);
    if(raw) aiLevel = { ...raw, source:"ai" };
  }
...
module.exports = { classifyLevel, mergeLevelDecision, resolveMessageLevel, aiLevelClassifierEnabled, assessLevelLLM, resolveMessageLevelAsync, canAttachMiniProgram, adviceOnlyReply, needsDoctorFromTriggers, handleIncoming, listSessions, sessionDetail, confirmDecision, updateSessionStatus, addNote, scanRisk, classifyIntent, retrieveKnowledge, retrieveKnowledgeVector, retrieveKnowledgeLocal, embedTexts, rerankDocs, cosine, knowledgeContentHash, normalizeDecision, rankMax, combineRisk, coerceRiskAssessment, assessRiskLLM, sentinelRaise, consultEntryResponses,
