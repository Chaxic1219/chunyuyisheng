...
const { db, hashPw, resolvePatient, mergePersons, mergePatients, autoMergePatientsByUserId, reconcileVerifiedPhonePersons, reconcileQiweIdentityPersons, afterDoctorProvisioned, friendlyPatientLabel, patientArchiveLabel, hydrateAdminMessageRow, decorateAdminPatient, allocateStaffId, stripChannelSuffix, isPlaceholderDisplayName, resolvePersonWechatName, preferDisplayName, patientArchive } = require("./db.js");
...
function ensureContactProfileSchema(){
  ...
  for(const row of rows){
    ...
    const needsMigrate = fields.length < 11
      || !keys.has("gender")
      || !keys.has("birthDate")
