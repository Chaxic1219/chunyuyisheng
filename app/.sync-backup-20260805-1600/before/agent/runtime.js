...
const { agentDryRun, healthChatEnabled } = require("./flags.js");
...
async function runAgentPath(input){
  ...
  const resolvedEarly = await triage.resolveMessageLevelAsync(text, doctorId, {});
  ...
  const resolved = await triage.resolveMessageLevelAsync(text, doctorId, {});
  ...
  if(planned.intendedAction === "health_chat"){
    ...
    const hcRisk = evaluateRisk(understood, session, {
