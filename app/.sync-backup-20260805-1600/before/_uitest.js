...
(async()=>{
  ...
  form.querySelector('[data-f="gender"] select').value="男";
  form.querySelector('[data-f="birthDate"] input').value="1990-01-01";
  ...
  // 家庭代办切换（姓名 → 患者姓名，避免与代办人姓名混淆）
  const nameLab=form.querySelector('[data-f="name"] > label');
  const nameTxt=()=>Array.from(nameLab.childNodes).find(n=>n.nodeType===3);
  doc.querySelector('#proxySeg [data-who="proxy"]').click();
  ok(!doc.querySelector("#proxyFrm").hidden, "家庭代办：切换显示代办人字段");
  ok(nameTxt() && nameTxt().textContent.trim()==="患者姓名", "家庭代办：姓名字段改为「患者姓名」");
  doc.querySelector('#proxySeg [data-who="self"]').click();
  ok(doc.querySelector("#proxyFrm").hidden, "切回本人办理：隐藏代办人字段");
  ok(nameTxt() && nameTxt().textContent.trim()==="患者姓名", "切回本人：加号页原标签「患者姓名」保持");
  ...
  const fenr=await (await globalThis.fetch(BASE+"/api/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({doctorId:1,type:"联络表",phone:fphone,code:fc1.code,consent:true,payload:{姓名:"随访UI",性别:"男",出生日期:"1990-01-01",手机号:fphone,主要疾病:"胃息肉",随访方案:"胃肠镜检查后随访",["检查/治疗日期"]:fdate}})})).json();
