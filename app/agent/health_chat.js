...
function needsKnowledge(text){
  return /能吃|可以吃|忌口|术后|复查|护理|注意事项|怎么喝|喝什么|饮食|能喝|喝牛奶|吃鸡蛋|副作用|不良反应|停药|换药|减量|加量|喝酒|饮酒|酒局|酒精|饭前|饭后|空腹|随餐|同时吃|一起吃|间隔|联合|中西药|汤剂|中成药/.test(String(text || ""));
}
...
function decidePhase({ understood, slots, evidence, emergency, clinicalRisk, chatPhaseHint, text }){
  ...
  // 向医生说话/汇报用药调整（呼格主任/医生 + 用药动作）→ 转医生，不生成科普：
  // 「主任 帕罗西汀订今晚改吃一片」「医生 今天开的中成药和西药汤剂明天取」「周六酒局帕罗西汀要不要停药」
  const directedToDoctor = /(^|\s|[,，:：、])(主任|医生|大夫|老师|X主任|周主任|吕主任|王主任)[，,、:：\s]/.test(t)
    || /^(主任|医生|大夫)[，,、:：\s]/.test(t);
  const medAdjust = /(帕罗西汀|抗抑郁|药|汤剂|中成药|西药|中药)[^。！？\n]{0,12}(停药|减量|加量|改吃|改量|换药|吃一片|两片|剂量|饭前|饭后|酒|同时|一起吃|衔接|明天取)/.test(t)
    || /(停药|减量|加量|改吃|换药|吃一片|两片|剂量)[^。！？\n]{0,12}(帕罗西汀|药|汤剂|中成药|西药|中药)/.test(t);
  if(directedToDoctor && medAdjust) return "escalate";
  if(medAdjust && /帕罗西汀|抗抑郁|抗焦虑|精神类|镇静|安眠/.test(t)) return "escalate";
  // 处方变更类问询：停/减/加/换/改剂量、是否继续服用 —— 改变用药方案是医生的决定权，
  // 与知识库有无证据无关，也不要求呼格医生，一律转医生。
  // 补 medAdjust 字面词表覆盖不到的口语化说法（「能停吗」「减到半片」「还要不要继续吃」）：
  // 此前这类问句落 intake，反问「哪里最不舒服」，既没答也没转。
  // 动作词按「动作+对象」成对匹配，避免「加重」「减轻」「改善」等同字异义误转。
  const hasDrugWord = /药|帕罗西汀|汤剂|中成药|西药|中药|处方/.test(t);
  const rxChangeAction = /[能要想该可否]{1,2}停|停[药了掉]|停不停/.test(t)
    || /减量|减到|减半|减成|减.{0,2}片/.test(t)
    || /加量|加到|加大|加.{0,2}片|加一点|加点/.test(t)
    || /换药|换成|换个|换一/.test(t)
    || /改吃|改成|改到|改量|改用/.test(t)
    || /半片|几片/.test(t)
    || /还(要不要|用不用|需不需要)(继续)?[吃服用]|要不要继续[吃服用]|还(继续)?吃吗|继续吃吗/.test(t);
  if(hasDrugWord && rxChangeAction) return "escalate";
...
async function resolveTurn(input){
  ...
  // 首次给建议须人工确认开关（HEALTH_CHAT_FIRST_REVIEW=1）：
  // 会话内尚未 deliver 过建议（!adviceDelivered）且本轮要 advise → 标记 needHumanReview，
  // 上层 risk 据此走 review（pending 人工确认），避免无复核医疗建议自动发出。
  const firstAdviceReview = !!(input.firstAdviceReview
    && phaseOut === "advise"
    && !slots.adviceDelivered);
  let attachCode = null;
  ...
  return {
    ...
    needHumanReview: firstAdviceReview
  };
}
