...
const { gawandeMethodPrompt } = require("./gawande_baseline.js");
...
async function composeViaFetch(input){
  ...
  const system = [
    ...
    withCard ? "本轮系统可能附带服务入口，正文里轻轻提一句即可，不要推销。"
      : ((tone === "advice" || tone === "handoff_soft" || goal === "advice" || goal === "health_report" || goal === "case_analysis" || goal === "care_plan" || goal === "consult")
          ? "若给出的是较全面的建议或就医/问诊指引，结尾可自然带一句：想一对一咨询请回复 101；不要生硬推销，服务类（挂号/加号等）不强行引导。"
          : "不要提小程序/服务入口/发编号。"),
    personaRules,
    "语调：" + tone + "；目标：" + goal + "。"
  ].filter(Boolean).join("\n");
  ...
  // 指数退避重试：仅对可恢复故障（5xx/超时/网络错误）重试，4xx（密钥/限流等）不重试。
  const maxRetries = 0;
  let lastErr = null;
  for(let attempt = 0; attempt <= maxRetries; attempt++){
    ...
    try{
      ...
      lastErr = { kind:"http_" + res.status, error:Object.assign(new Error("http_" + res.status), { llmRetryable:true }) };
      // 4xx 不重试
    }catch(e){
      clearTimeout(timer);
      lastErr = { kind:(e && e.name === "AbortError") ? "timeout" : "fetch_error", error:e };
      if(e) e.llmRetryable = true;
      if(lastErr.kind === "fetch_error"){
        // fetch 抛出即代表网络级失败，统一视为可恢复
      }
    }
  ...
  if(lastErr && lastErr.error){ lastErr.error.reason = lastErr.kind; throw lastErr.error; }
