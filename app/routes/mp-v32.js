...
function registerMpV32Routes(route, ctx) {
  ...
  route("GET", /^\/api\/mp\/v32\/services$/, (req, res, _m, q) => {
    // 服务列表允许静默浏览（PRD 9.1）：未登录也可查看。
    // 医生上下文优先级：query.doctorId > 会话 doctor_id（避免未传医生时串出种子医生商品）
    let doctorId = null;
    const qDoctor = q && q.doctorId != null && String(q.doctorId).trim() !== "" ? +q.doctorId : NaN;
    if (Number.isFinite(qDoctor) && qDoctor > 0) doctorId = qDoctor;
    if (!doctorId) {
      try {
        const token = bearerToken(req);
        if (token) {
          const sess = mpAuth.requireSession(token);
          if (sess && sess.doctor_id) doctorId = +sess.doctor_id;
        }
      } catch (e) {
        /* token 无效 → 按未登录浏览 */
      }
    ...
    try {
      ...
      // 必须带医生才拉商品：无医生上下文时返回空列表，由前端展示「筹备中」
      products = doctorId ? svc.catalog.buildServiceCenterProducts(doctorId) : [];
    } catch (e) {
      products = [];
    }
    json(res, 200, {
      data: {
        ...mpV32.getServiceCenter(doctorName, { products }),
  ...
  route("DELETE", /^\/api\/mp\/v32\/family\/(\d+)$/, (req, res, m) => {
    const sess = requirePerson(req, res);
    if (!sess) return;
    const memberId = Number(m[1]);
    const result = mpV32.repo.removeFamily(+sess.person_id, memberId);
    if (!result.removed) {
      json(res, 404, { error: "家属不存在或已删除" });
      return;
    }
    json(res, 200, {
      data: {
